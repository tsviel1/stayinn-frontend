<template>
  <section>
    ...
    <form @submit.prevent="onFilterBeds($event)">
      <p>Bedrooms</p>
      <input type="radio" id="room-one" value="1" v-model.number="filterBy.bedrooms" @change="onFilterBedroom($event)" />
      <label for="room-one">1</label>

      <input type="radio" id="room-two" value="2" @change="onFilterBedroom($event)" v-model.number="filterBy.bedrooms" />
      <label for="room-two">2</label>
      <input type="radio" id="room-three" value="3" @change="onFilterBedroom($event)" v-model.number="filterBy.bedrooms" />
      <label for="room-three">3</label>

      <input type="radio" id="room-four" value="4" @change="onFilterBedroom($event)" v-model.number="filterBy.bedrooms" />
      <label for="room-four">4</label>
      <input type="radio" id="room-five" value="5" @change="onFilterBedroom($event)" v-model.number="filterBy.bedrooms" />
      <label for="room-five">5</label>

      <input type="radio" id="room-six" value="6" @change="onFilterBedroom($event)" v-model.number="filterBy.bedrooms" />
...
export default {
  ...
  methods: {
    ...
    onFilterBeds(ev) {
      ...
      console.log(this.filterBy.beds);
    },
    onFilterPrice(ev) {
      console.log(ev);
      const price = {
